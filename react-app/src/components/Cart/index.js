...
function Cart() {
  ...
  const updateQuantity = async (itemId, quantity) => {
    ...
    Object.values(cartUlItems).forEach((ulItem) =>
      ulItem.classList.remove("disable-hover")
    );
  };

  useEffect(() => {
    const lastItem = document.getElementById(`cart-ul-${cartItems.length - 1}`);
    if (lastItem) lastItem.classList.add("cart-last-item-ul");
  });
  ...
  const handleSubmit = async (e) => {
    ...
    const dropDownContainer = document.getElementById(
      `item-${currentItem}dropdown`
    );
    const customQuantityContainer = document.getElementById(
      `item-${currentItem}custom`
    );
